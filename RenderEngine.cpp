...
bool CRenderEngine::Initialize(HDC hContext_i)
{
	...
	if (!tex.CreateFromFile(L"media/mri-head.raw", 256, 256, 256))
...
void CRenderEngine::initFonts()
{
	...
	hfont.CreateFont(32, 0, 0, 0, FW_EXTRABOLD, 0, 0, 0, 0, 0, 0, 0, 0, L"Times New Roman");
	//"Proxy 1");
...
void CRenderEngine::TextureVR()
{
	...
	for (float fIndx = -1.0f; fIndx <= 1.0f; fIndx += 0.1f)
	{
		glBegin(GL_QUADS);
